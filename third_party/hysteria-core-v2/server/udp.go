...
func writeToWithRedundancy(conn UDPConn, data []byte, addr string, multiplier int) (int, error) {
	...
	for i := 0; i < multiplier; i++ {
		written, err := conn.WriteTo(data, addr)
		if err != nil {
			return written, err
		}
		if i == 0 {
...
func (e *udpSessionEntry) receiveLoop() {
	...
	for {
		...
		err = e.IO.LogReceive(len(msg.Data))
		if err != nil {
			e.CloseWithErr(err)
			return
		}
...
func sendMessageAutoFragWithRedundancy(io udpIO, buf []byte, msg *protocol.UDPMessage, multiplier int) error {
	for i := 0; i < multiplier; i++ {
		if err := sendMessageAutoFrag(io, buf, msg); err != nil {
			return err
		}
	}
	return nil
